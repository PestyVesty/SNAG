...
class GraphNet(torch.nn.Module):
    ...
    def evalate_actions(self, actions, state_num):
        ...
        # print('evaluate actions:',actions, state_num)
#         print('evalate_actions:',actions,state_length,state_num)
    ...
    def evaluate_structure(self, actions, layer_nums, state_num=6):
        ...
        for i in range(layer_nums):
            head_num = actions[i * state_num + 4]
    ...
    def build_hidden_layers(self, actions, batch_normal, drop_out, layer_nums, num_feat, num_label, state_num=6):
        ...
        # build hidden layer
        #sample 1head_num + 1channels + final jk_mode for this architectures
        act = "relu"
        jk_mode = actions[state_num + 3]
        head_num = actions[state_num + 4]
        out_channels = actions[state_num + 5]
        ...
        for i in range(layer_nums):
            ...
            use_skip = actions[i * state_num + 2]
            ...
            self.layers.append(
                NASLayer(attention_type, aggregator_type, act, head_num, in_channels,  out_channels,
                                    dropout=drop_out, concat=concat, residual=residual, batch_normal=batch_normal)
            )
        # process skip_op for final_layers
        # final_layers = []
        # final_layers += [self.layers.pop()]
        # for i in range(layer_nums-1):
        #     # final layers already used.
        #     use_skip = actions[i * state_num + 2]
        #
        #     if use_skip == 'none':
        #         continue
        #     else:
        #         final_layers += [self.layers[i]]
        #
        # if jk_mode == 'concat':
        #     jk_func = JumpingKnowledge(mode='cat')
        #     final_layers = jk_func(final_layers)
        #     # if final_layers.shape()[-1] != out_channels:
        #     #     final_trans = torch.nn.Linear(final_layers.shape()[-1], out_channels)
        #     #     final_layers = final_trans(final_layers)
        # if jk_mode == 'lstm':
        #     jk_func = JumpingKnowledge(mode='lstm',channels=out_channels*head_num, num_layers=layer_nums)
        #     final_layers = jk_func(final_layers)
        # if jk_mode == 'maxpool':
        #     jk_func = JumpingKnowledge(mode='max')
        #     final_layers = jk_func(final_layers)
        #
        # #final transfer from channels to num_of_class
        # final_lin = torch.nn.Linear(final_layers.shape()[-1], num_label)
        # final_layers = final_lin(final_layers)
        # self.layers.append(final_layers)
